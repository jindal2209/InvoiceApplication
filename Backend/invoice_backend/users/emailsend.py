...
def createInvoicePDF(customer_name,invoice_id,invoice_date,company_name,customer_email,invoice_items_list,total_amount):
    ...
    col_width = pdf.epw / len(TABLE_COL_NAMES)  # distribute content evenly
    font_face = 'times'
    logo_path = '{p}\\logo.png'.format(p=Path(__file__).resolve().parent.parent)
    pdf_path = '{p}\\generated_pdf\\INVOICE_{pdf_name}.pdf'.format(pdf_name=invoice_id,p=Path(__file__).resolve().parent.parent)
...
def sendMail(invoice_id, company_name, invoice_items_list, customer_name, customer_email_id, total_amount,invoice_date):
    ...
    body = '''
            ...
            Kindly pay the pending amount using this link: http://localhost:3000/#/invoice/{invoice_id}
...
def sendPaymentConfirmationMail(invoice_id,customer_name,customer_email_id,total_amount):
    ...
    body = '''
            ...
            You can check status using this link: http://localhost:3000/#/invoice/{invoice_id}
        '''.format(customer_name=customer_name,total_amount=total_amount,invoice_id=invoice_id)
